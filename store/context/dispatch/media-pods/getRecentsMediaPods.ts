...
export async function getRecentsMediaPods(dispatch: Dispatch<MediaPodActionTypes>, client: ApiClient): Promise<null | GetMediaPods> {
  try {
    dispatch({ type: MediaPodAction.MEDIA_PODS_LOADING_START });

    const response = await client?.getRecentsMediaPods();

    if (response === null || response === undefined) {
      dispatch({
        type: MediaPodAction.GET_RECENTS_MEDIA_PODS_HTTP_INTERNAL_ERROR,
    ...
    switch (response.status) {
      ...
      case HttpStatus.NOT_FOUND:
        dispatch({
          type: MediaPodAction.GET_RECENTS_MEDIA_PODS_NOT_FOUND,
    ...
    dispatch({
      type: MediaPodAction.GET_RECENTS_MEDIA_PODS_ERROR,
